...
create_virtual_keyboard_device (MetaWaylandCompositor *compositor)
{
  ...
  ClutterSeatClass *klass;
  ClutterInputDevice *real_keyboard;
  ...
  /*
   * native backends usually have a "real" keyboard device while nested backends may not.
   * if we have a real keyboard, create a virtual device like it
   * if we don't, fall back to creating a virtual keyboard device directly.
   */
  real_keyboard = clutter_seat_get_keyboard (seat);
  if (real_keyboard)
    return create_virtual_device_like (seat, real_keyboard);
