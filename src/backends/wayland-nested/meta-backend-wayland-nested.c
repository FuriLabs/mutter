                                                 ...
                                                 GError      **error)
{
  ...
  if (!priv->xkb_keymap) {
    ...
    if (!priv->keymap_description)
      priv->keymap_description = meta_keymap_description_new_from_rules ("pc105",
                                                                         "us",
                                                                         NULL,
                                                                         NULL,
                                                                         NULL,
  ...
  if (priv->xkb_keymap && META_IS_SEAT_WAYLAND_NESTED (priv->seat))
    meta_seat_wayland_nested_set_keymap (META_SEAT_WAYLAND_NESTED (priv->seat),
                                         priv->xkb_keymap,
...
meta_backend_wayland_nested_get_input_settings (MetaBackend *backend)
{
  ...
  if (!priv->input_settings)
    priv->input_settings = g_object_new (META_TYPE_INPUT_SETTINGS_WAYLAND_NESTED,
                                         "backend", backend,
                                         NULL);
                                              ...
                                              GTask                 *task)
{
  ...
  new_keymap = meta_keymap_description_create_xkb_keymap (description,
                                                          NULL,
                                                          NULL,
                                                          &error);
  if (!new_keymap) {
  ...
  priv->xkb_layout_index = layout_index;
  priv->keymap_description = meta_keymap_description_ref (description);
...
static struct xkb_keymap *
meta_backend_wayland_nested_get_xkb_keymap (MetaBackend *backend)
{
  MetaBackendWaylandNestedPrivate *priv = meta_backend_wayland_nested_get_instance_private (META_BACKEND_WAYLAND_NESTED (backend));
...
meta_backend_wayland_nested_get_keymap_description (MetaBackend *backend)
{
  MetaBackendWaylandNestedPrivate *priv = meta_backend_wayland_nested_get_instance_private (META_BACKEND_WAYLAND_NESTED (backend));

  if (!priv->keymap_description)
    meta_backend_wayland_nested_get_xkb_keymap (backend);

  return priv->keymap_description;
}
...
static gboolean
meta_backend_wayland_nested_is_headless (MetaBackend *backend)
{
  (void) backend;
  return TRUE;
}
...
meta_backend_wayland_nested_class_init (MetaBackendWaylandNestedClass *klass)
{
  ...
  backend_class->get_xkb_keymap = meta_backend_wayland_nested_get_xkb_keymap;
  backend_class->get_keymap_description = meta_backend_wayland_nested_get_keymap_description;
  ...
  backend_class->is_headless = meta_backend_wayland_nested_is_headless;
