...
meta_clutter_backend_wayland_nested_get_default_seat (ClutterBackend *clutter_backend)
{
  MetaClutterBackendWaylandNested *self =
    META_CLUTTER_BACKEND_WAYLAND_NESTED (clutter_backend);
...
meta_clutter_backend_wayland_nested_get_cursor (ClutterBackend    *clutter_backend,
                                                ClutterCursorType  cursor_type)
{
  MetaClutterBackendWaylandNested *self = META_CLUTTER_BACKEND_WAYLAND_NESTED (clutter_backend);

  return meta_backend_get_cursor (self->backend, cursor_type);
}
                       ...
                       ClutterStage                    *stage)
{
  ...
  seat = meta_backend_get_default_seat (self->backend);

  if (seat && META_IS_SEAT_WAYLAND_NESTED (seat))
    device = meta_seat_wayland_nested_peek_pointer (META_SEAT_WAYLAND_NESTED (seat));

  self->pointer_sprite = g_object_new (META_TYPE_SPRITE_WAYLAND_NESTED,
                                       "backend", self->backend,
                                       "stage", stage,
                                                ...
                                                const ClutterEvent *for_event)
{
  ...
  ClutterInputDeviceType t;

  source_device = clutter_event_get_source_device (for_event);
  if (!source_device)
    return NULL;

  t = clutter_input_device_get_device_type (source_device);
                                                   ...
                                                   ClutterEventSequence *sequence)
{
  ...
  t = clutter_input_device_get_device_type (device);
  if (t == CLUTTER_KEYBOARD_DEVICE || t == CLUTTER_PAD_DEVICE)
    return NULL;
...
meta_clutter_backend_wayland_nested_class_init (MetaClutterBackendWaylandNestedClass *klass)
{
  ...
  clutter_backend_class->get_cursor = meta_clutter_backend_wayland_nested_get_cursor;
