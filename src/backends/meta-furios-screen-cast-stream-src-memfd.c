...
destroy_pbos (MetaFuriosScreenCastStreamSrcMemfd *self)
{
  ...
  if (self->pbo_ids[0] || self->pbo_ids[1]) {
    self->cogl_context->glDeleteBuffers (2, self->pbo_ids);
    self->pbo_ids[0] = 0;
    self->pbo_ids[1] = 0;
  }
                           ...
                           graphene_rect_t                    *out_rect)
{
  ...
  MetaCursorSprite *cursor_sprite = meta_cursor_renderer_get_cursor (cursor_renderer);
  if (!cursor_sprite)
    return FALSE;

  meta_cursor_sprite_realize_texture (cursor_sprite);

  graphene_rect_t rect = meta_cursor_renderer_calculate_rect (cursor_renderer, cursor_sprite);
...
ensure_cursor_tracking (MetaFuriosScreenCastStreamSrcMemfd *self)
{
  ...
  meta_cursor_tracker_track_position (self->cursor_tracker);
             ...
             GError                            **error)
{
  ...
  if (self->pbo_ids[0] == 0 && self->pbo_ids[1] == 0) {
    self->cogl_context->glGenBuffers (2, self->pbo_ids);
    self->pbo_size = 0;
    self->pbo_write_index = 0;
    self->pbo_have_previous = FALSE;
  }
  ...
  if (self->pbo_size != required_size) {
    self->cogl_context->glBindBuffer (GL_PIXEL_PACK_BUFFER, self->pbo_ids[0]);
    self->cogl_context->glBufferData (GL_PIXEL_PACK_BUFFER,
                                      (GLsizeiptr) required_size,
                                      NULL,
                                      GL_STREAM_READ);

    self->cogl_context->glBindBuffer (GL_PIXEL_PACK_BUFFER, self->pbo_ids[1]);
    self->cogl_context->glBufferData (GL_PIXEL_PACK_BUFFER,
                                      (GLsizeiptr) required_size,
                                      NULL,
                                      GL_STREAM_READ);

    self->cogl_context->glBindBuffer (GL_PIXEL_PACK_BUFFER, 0);
                                   ...
                                   GError                            **error)
{
  ...
  self->cogl_context->glPixelStorei (GL_PACK_ALIGNMENT, 1);

  const int write_i = self->pbo_write_index;
  const int read_i = 1 - write_i;

  self->cogl_context->glBindBuffer (GL_PIXEL_PACK_BUFFER, self->pbo_ids[write_i]);

  cogl_framebuffer_flush (fb);

  self->cogl_context->glReadPixels (0, 0, width, height,
                                    GL_BGRA, GL_UNSIGNED_BYTE, (void *) 0);

  const int map_i = self->pbo_have_previous ? read_i : write_i;
  self->cogl_context->glBindBuffer (GL_PIXEL_PACK_BUFFER, self->pbo_ids[map_i]);

  const guint8 *src = NULL;

  if (self->cogl_context->glMapBufferRange)
    src = (const guint8 *) self->cogl_context->glMapBufferRange (GL_PIXEL_PACK_BUFFER,
                                                                 0,
                                                                 (GLsizeiptr) required,
                                                                 GL_MAP_READ_BIT);
  else if (self->cogl_context->glMapBuffer)
    src = (const guint8 *) self->cogl_context->glMapBuffer (GL_PIXEL_PACK_BUFFER,
                                                            GL_READ_ONLY);
  ...
  if (self->cogl_context->glUnmapBuffer)
    self->cogl_context->glUnmapBuffer (GL_PIXEL_PACK_BUFFER);
